...
vim.keymap.set('n', '<leader>p', function() fzf_lua.files({ cwd = require('markhilb.utils').git_root() }) end)
vim.keymap.set('n', '<leader>P', fzf_lua.files)
vim.keymap.set(
    'n',
    '<leader>g',
    function() fzf_lua.live_grep_native({ cwd = require('markhilb.utils').git_root() }) end
)
...
vim.keymap.set('n', '<leader>ts', fzf_lua.search_history)
